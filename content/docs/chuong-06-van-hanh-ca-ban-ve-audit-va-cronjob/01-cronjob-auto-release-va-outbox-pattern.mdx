...
- **Vì sao ghi `OUTBOX_MESSAGES` vào DB thay vì bắn thẳng vào Queue?**: Nếu DB commit thành công nhưng Queue broker (Redis/RabbitMQ) bị sập, tin nhắn sẽ bị mất. Lưu vào bảng DB `OUTBOX_MESSAGES` cùng Transaction đảm bảo tính toàn vẹn 100% (Guaranteed Delivery).
