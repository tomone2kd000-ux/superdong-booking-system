...
| **4** | **Sơ Đồ Chọn Ghế (Seat Map)** | Trực quan hóa tầng (Deck), khoang (Zone) tàu. Khách bấm chọn vị trí ghế cụ thể cho từng lượt. | `POST /api/v1/bookings/{id}/hold-seats` -> Kích hoạt **MySQL Lock (`DB::transaction` & `lockForUpdate`)** để giữ ghế chống bán trùng, cập nhật `held_seats` trong 15 phút. |
